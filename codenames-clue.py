...
while len(board['blue']) != 1 and len(board['red']) != 0 :
    ...
    for i in range(len(clue_b)):

        for j in range(len(blue)):

            sim += vector_similarity(nlp.vocab[clue_[i]].vector, nlp.vocab[blue[j]].vector)
    ...
    while True:
        ...
        for i in range(len(words)):
            ...
            sim = vector_similarity(nlp.vocab[clue_b].vector, nlp.vocab[words[i]].vector)
    ...
    for i in range(len(clue_r)):

        for j in range(len(red)):
            sim += vector_similarity(nlp.vocab[clue_[i]].vector, nlp.vocab[red[j]].vector)
    ...
    while True:
        ...
        print("ENTER THE ANSWER FOR RED")
        sim_0 = 0
        ans = words[0]

        for i in range(len(words)):

            sim = vector_similarity(nlp.vocab[clue_r].vector, nlp.vocab[words[i]].vector)
